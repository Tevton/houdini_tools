...
def create_render_node_network(out_node, reference_node_pos, offset, flag):
    ...
    if hou.node("/obj/" + render_node_name) is not None:
        for node in hou.node("/obj/" + render_node_name).children():
                ...
                if node.type().name() == "object_merge":
                    ...
                    if not flag and objmerge_node.parm("objpath1").eval() != out_node.path():
                        ...
                        buttons = ('YES', 'YES for all', 'Skip', 'Cancel'), close_choice = 3)
    ...
    # Creating node setup in render node  
        ...
        new_objmerge_node.setPosition(hou.Vector2(get_null_node_pos[0], get_null_node_pos[1] + 3))
...
for el in hou.selectedNodes():
    if el.parent().name() != 'obj':
        hou.ui.displayMessage("Please select nodes only in /obj context", title = "WARNING")
        exit()
else:
    ...
    result, values = hou.ui.readMultiInput("Please specify prefixes:",("YOUR output prefix", "Render Node prefix"), 
                      buttons = ("OK", "Cancel"), close_choice = 1, title = "Render nodes Setup", 
    ...
    if result == 1: # 'Cancel'
        ...
        reference_node_pos = hou.selectedNodes()[0].position() # Position for creating new nodes
        ...
        for node in hou.selectedNodes():
            ...
            if node.type().name() == "geo" and not node.name().upper().startswith(out_prefix):
                ...
                for child in node.children():
                    if child.type().name() == "null":
                        if child.name().upper().startswith(out_prefix):
                            yes_for_all3 = create_render_node_network(child, reference_node_pos, offset, yes_for_all3)
                            offset -= 1
                        if not child.name().upper().startswith(out_prefix) and min_child_pos[1] == child.position()[1]:
                            if not yes_for_all:  # Show dialog only if first "YES for All" is not active
                                result = hou.ui.displayMessage(
                                ...
                                title = "WARNING")
                                if result == 1: # "YES for all"
                                    yes_for_all = True # Flag to track first "YES for All" state
                            if result == 2: # Skip current node
                                continue
                            if result == 0 or result == 1: # Automatically apply if first "YES for All" is active
                                new_null_node = setup_children_null(child)
                                yes_for_all3 = create_render_node_network(new_null_node, reference_node_pos, offset, yes_for_all3)
                                offset -= 1
                            if result == 3: # "Cancel"
                                exit()
                    
                    # Find bottom nodes position and ask user to create null nodes under them
                    if min_child_pos[1] == child.position()[1] and child.type().name() != "null":
                        if not yes_for_all2:  # Show dialog only if second "YES for All" is not active
                            result = hou.ui.displayMessage(
                                    ...
                                    title = "WARNING")
                            if result == 1: # "YES for all"
                                yes_for_all2 = True # Flag to track second "YES for All" state
                        if result == 2: # Skip current node
                            continue
                        if result == 0 or result == 1: # Automatically apply if second "YES for All" is active
                            new_null_node = setup_children_null(child)
                            yes_for_all3 = create_render_node_network(new_null_node, reference_node_pos, offset, yes_for_all3)
                            offset -= 1
                        if result == 3: # "Cancel"
