...
def create_sop_import_node(node, reference_node_pos, offset, flag):
    ...
    if sop_import_node_name.upper().startswith("RENDER_") or sop_import_node_name.upper().startswith("OUT_"):
        sop_import_node_name = sop_import_node_name[sop_import_node_name.find("_") + 1:]
    ...
    # If the node doesn't exist, create a new SOP Import node
        ...
        new_sopimport_node.setPosition(hou.Vector2(reference_node_pos[0] + 10, reference_node_pos[1] + offset))
